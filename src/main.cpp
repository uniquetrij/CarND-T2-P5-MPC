...
int main0()
{
                ...
                uWS::OpCode opCode)
    {
        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
                ...
                // transformation to vehicle's orientation
                ...
                // cross track error
                double cte = coeffs[1]; // polyeval_derivative(coeffs, 0.0);

                // orientation error
                double epsi = -atan(coeffs[1]); // -atan(polyeval_derivative(coeffs, 0.0));
                ...
                // prepare latency state vector
                Eigen::VectorXd state(6);
                state << latency_px, latency_py, latency_psi, latency_v, latency_cte, latency_epsi;

                // solve for actuation values
